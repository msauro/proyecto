...
class AdminController extends Cmx_Base {
    ...
    public function addAction(){
		if($this->getRequest()->isPost()){
			...
			if($alreadyRegistered instanceof Exception){
                Cmx_Base::addError($alreadyRegistered->getMessage());
            ...
            // If this email wasn't registered in the past -> add a new admin
            	...
            	Cmx_Base::addSuccess('Administrator succesfully added');
	...
	public function editAction() {
		if($this->getRequest()->isPost()){
			...
			if($result instanceof Exception){
            	...
            	Cmx_Base::addSuccess('Administrator succesfully updated');
            }
		...
		if($id_admin){
			if($id_admin != $this->admin_session->admin['id_admin']){
				...
				if($result instanceof Exception){
					...
					Cmx_Base::addSuccess('Administrator succesfully deleted');
				}
			}else{
				Cmx_Base::addError('You cannot delete yourself');
			}
		}
		$this->_redirect('admin/list');
	}
	...
	public function isemailavailableAction(){
		if($this->getRequest()->isPost()){
			...
			if($isAvailable instanceof Exception){
				Cmx_Base::addError("There was an error validating your e-mail address.");
			}else{
